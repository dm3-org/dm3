...
import { getAvatarProfilePic, getEnsProfileDetails } from "../../utils/ens-utils";
...
export function Profile() {
  ...
  const fetchUserEnsProfileDetails = async () => {
    const ensDetails: EnsProfileDetails = await getEnsProfileDetails(state, state.connection.account?.ensName as string);
  ...
  return (
    <>
      ...
      <div className="profile-details-container text-primary-color">
        ...
        <img src={profilePic ? profilePic : profPic} alt="profile-pic" className="border-radius-4 profile-image" />

        <div className="profile-detail-items mt-3">

          <EnsDetails propertyKey={"Name"} propertyValue={state.connection.account?.ensName as string} />
          <EnsDetails propertyKey={"Address"} propertyValue={state.connection.ethAddress as string} />
          ...
          <div className="ens-btn-container">
            <Button
              ...
              actionMethod={() => openEnsProfile(state.connection.account?.ensName as string)}
            />
          </div>
          ...
          <div className="configure-btn-container">
            <Button
              ...
              actionMethod={() => openProfileConfigureBox(dispatch)}
            />
          </div>
